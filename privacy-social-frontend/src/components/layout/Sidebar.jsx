...
const NavItem = ({ to, icon, label, isActive, badge }) => (
    <Link
        ...
        className={`flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-300 group relative overflow-hidden ${isActive
            ? 'bg-primary-500/10 text-primary-500 font-semibold shadow-[0_0_20px_rgba(59,130,246,0.3)] border border-primary-500/20 scale-[1.02]'
            : 'text-text-secondary hover:bg-white/10 hover:text-text-primary hover:pl-5 hover:scale-[1.01]'
            }`}
    >
        {/* Active Glow Effect */}
        {isActive && (
            <div className="absolute inset-0 bg-gradient-to-r from-primary-500/10 via-purple-500/10 to-primary-500/10 animate-pulse"></div>
        )}
        ...
        <span className="text-sm md:text-base flex-1 relative z-10">{label}</span>

        {badge > 0 && (
            <span className="relative z-10 bg-red-500 text-white text-[10px] font-bold px-1.5 py-0.5 rounded-full min-w-[18px] text-center shadow-lg shadow-red-500/40 animate-pulse group-hover:scale-110 transition-transform">
                {badge > 99 ? '99+' : badge}
            </span>
        )}
